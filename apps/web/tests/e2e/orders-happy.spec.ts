...
test.describe('Orders happy path', () => {
  test.skip(
    !hasCreds,
    'E2E_EMAIL and E2E_PASSWORD must be set to run this test'
  );
  ...
  test('login and list orders', async ({ page }) => {
    ...
    await expect(
      page.locator('h1:has-text("Zamówienia"), h1:has-text("Moje zamówienia")')
    ).toBeVisible();
    // If there are orders, an item card should exist; otherwise, empty state should be visible.
    const anyOrder = page.locator(
      '[data-testid="order-card"], text="Brak zamówień"'
    );
