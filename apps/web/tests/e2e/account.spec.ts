...
test.describe('Account flows', () => {
  test('redirects unauthenticated users from /moje-konto to home or shows login modal', async ({
    page,
  }) => {
    ...
    const loginModal = page.locator(
      '[data-testid="auth-modal"], [role="dialog"]'
    );
    ...
    if (modalVisible) {
      ...
      expect(
        url.endsWith('/') || url.endsWith('/index') || url.endsWith('/pl')
      ).toBeTruthy();
    }
  });
