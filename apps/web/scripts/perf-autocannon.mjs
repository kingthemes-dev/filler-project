...
const ENDPOINTS = [
  ...
  {
    name: 'Single Product',
    path: '/api/woocommerce?endpoint=products/433',
    method: 'GET',
    priority: 'P0',
    expectedP95: 400,
    expectedP99: 700,
...
async function testEndpoint(endpoint, scenario = 'warm') {
  ...
  // Warm-up: prime cache once for warm scenario
  if (scenario === 'warm') {
    try {
      ...
      log(`   🔥 Warm-up status: ${res.status}`, 'yellow');
    } catch (e) {
      log(`   ⚠️ Warm-up failed: ${e.message}`, 'red');
    }
  }
