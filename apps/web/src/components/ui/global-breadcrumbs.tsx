...
const HomeIcon: ComponentType<{ className?: string }> = props => (
  <Home {...props} />
);
const FileIcon: ComponentType<{ className?: string }> = props => (
  <FileText {...props} />
);
...
export default function GlobalBreadcrumbs() {
  ...
  const generateBreadcrumbs = () => {
    const breadcrumbs = [{ label: 'Strona główna', href: '/', icon: HomeIcon }];
    ...
    segments.forEach((segment, index) => {
      ...
      breadcrumbs.push({ label, href, icon: FileIcon });
    });
