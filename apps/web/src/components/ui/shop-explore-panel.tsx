    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          className="absolute top-full left-0 right-0 w-full bg-white/80 backdrop-blur-md border-l border-r border-b border-t-0 border-gray-200/50 z-50 rounded-b-3xl mx-auto"
          initial={{ opacity: 0, height: 0 }}
          animate={{ opacity: 1, height: 'auto' }}
          exit={{ opacity: 0, height: 0 }}
          transition={{ duration: 0.3, ease: 'easeOut' }}
        >
          <div className="max-w-[95vw] mx-auto px-4 sm:px-8 pt-8 pb-8 relative backdrop-blur-md">
            {/* Close button */}
            <button
              onClick={onClose}
