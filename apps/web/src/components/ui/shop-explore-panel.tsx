...
function ShopExplorePanelContent({ open, onClose }: ShopExplorePanelProps) {
  ...
  } = useShopAttributes(); // Użyj wszystkich marek zamiast brandsForModal
  ...
  useEffect(() => {
    if (open) {
      ...
      initialize()
        .then(() => {
          // Wait a bit for store to update and force re-render
          setTimeout(() => {
            const storeState = useShopDataStore.getState();
            console.log('✅ Store initialized:', {
              categories: storeState.categories.length,
              mainCategories: storeState.categories.filter(c => (c.parent || 0) === 0).length,
              brands: storeState.attributes.brands.length,
  ...
  const isCategoryHovered = (categoryId: string) =>
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        window.dispatchEvent(
          new CustomEvent('shopModalToggle', { detail: { open: false } })
        );
      }
    };
        ...
        <>
          ...
          >
            ...
            >
              ...
              <div
                className="flex-1 overflow-y-auto px-4 lg:px-6 pt-6 pb-8 relative"
                onClick={e => e.stopPropagation()}
              >
                <div
                  ref={panelRef}
                    ...
                    {categoriesLoading ? (
                      ...
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                          Kategorie będą dostępne wkrótce
                        </p>
                      </div>
                    ) : (
                      ...
                      <div className="space-y-2">
                        {mainCategories.map(category => {
                          ...
                          const isHovered = isCategoryHovered(
                          ...
                          return (
                            <motion.div
                              ...
                              onClick={e => e.stopPropagation()}
                              onMouseEnter={() =>
                                handleCategoryHover(category.id.toString())
                              }
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          {zastosowanie.map((term, index) => (
                            ...
                            >
                              <Link
                                href={buildFilterUrl({
                                  pa_zastosowanie: term.slug || term.name,
                                })}
...
export default function ShopExplorePanel({
  open,
  onClose,
}: ShopExplorePanelProps) {
  return (
