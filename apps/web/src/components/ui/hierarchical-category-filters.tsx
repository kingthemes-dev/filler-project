...
const mapWooCommerceCategories = (
  wooCategories: Array<{
    id: number;
    name: string;
    slug: string;
  ...
  > = {
    ...
    rewitalizacja: {
      mainCategory: 'mezoterapia',
      subcategory: 'rewitalizacja',
    },
    ...
    'hydroksyapatyt-wapnia-caha': {
      mainCategory: 'stymulatory',
      subcategory: 'hydroksyapatyt-wapnia',
    },
    kolagen: { mainCategory: 'stymulatory', subcategory: 'kolagen' },
    'kwas-bursztynowy': {
      mainCategory: 'stymulatory',
      subcategory: 'kwas-bursztynowy',
    },
    'kwas-hialuronowy': {
      mainCategory: 'stymulatory',
      subcategory: 'kwas-hialuronowy',
    },
    'kwas-polimlekowy-plla': {
      mainCategory: 'stymulatory',
      subcategory: 'kwas-polimlekowy',
    },
    'polikaprolakton-pcl': {
      mainCategory: 'stymulatory',
      subcategory: 'polikaprolakton',
    },
    polinukleotydy: {
      mainCategory: 'stymulatory',
      subcategory: 'polinukleotydy',
    },

    // Wypełniacze
    'korekcja-nosa': {
      mainCategory: 'wypelniacze',
      subcategory: 'korekcja-nosa',
    },
    'wypelniacze-ha-ogolne': {
      mainCategory: 'wypelniacze',
      subcategory: 'wypelniacze-ha',
    },
    'zmarszczki-srednie': {
      mainCategory: 'wypelniacze',
      subcategory: 'zmarszczki-srednie',
    },

    // Peelingi
    'post-peel-pielegnacja-pozabiegowa': {
      mainCategory: 'peelingi',
      subcategory: 'post-peel',
    },
  };
  ...
  wooCategories.forEach(wooCategory => {
    ...
    if (mapping) {
      const mainCategory = hierarchicalCategories.find(
        cat => cat.id === mapping.mainCategory
      );
      if (mainCategory) {
        if (mapping.subcategory) {
          const subcategory = mainCategory.subcategories.find(
            sub => sub.slug === mapping.subcategory
          );
          if (subcategory) {
  ...
  hierarchicalCategories.forEach(category => {
    if (category.subcategories.length > 0) {
      category.count = category.subcategories.reduce(
                  ...
                  {category.subcategories.map((subcategory, index) => (
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          handleCategoryClick(category.slug, subcategory.slug)
                        }
                        className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500 rounded"
                      />
                      <span className="ml-3 text-xs sm:text-sm font-medium text-gray-700 text-left">
                        {subcategory.name}
                      </span>
