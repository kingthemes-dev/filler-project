...
export default function KingProductTabsServer({
  data,
}: KingProductTabsServerProps) {
  ...
  const nowosci = useMemo(
    () =>
      data.nowosci && data.nowosci.length > 0
        ? data.nowosci
  ...
  const promocje = useMemo(() => {
    const promocjeRaw =
      data.promocje && data.promocje.length > 0 ? data.promocje : allProducts;
    ...
    const filtered = mergeUnique([promocjeRaw]).filter(p => {
      const hasSalePrice =
        p.sale_price && p.sale_price !== '' && p.sale_price !== '0';
      const hasOnSaleFlag = p.on_sale === true;
      const priceDiffers =
        p.price &&
        p.regular_price &&
        parseFloat(p.price) < parseFloat(p.regular_price);
  ...
  const polecane = useMemo(() => {
    const polecaneRaw =
      data.polecane && data.polecane.length > 0 ? data.polecane : allProducts;
    return mergeUnique([polecaneRaw])
      .filter(p => Boolean(p.featured))
      .slice(0, 8);
  }, [data.polecane, allProducts]);

  const bestsellery = useMemo(() => {
    const bestselleryRaw = Array.isArray(data.bestsellery)
      ? data.bestsellery
      : [];
  ...
  return (
    <section
      className="mt-6 py-12 sm:py-16 bg-white rounded-2xl sm:rounded-3xl overflow-hidden"
      ...
      <div className="px-0 sm:px-6 md:px-8 lg:px-12">
        ...
        <div className="mt-6">
          <AnimatePresence mode="wait">
            <div key={activeTab} role="tabpanel" aria-live="polite">
              {isTransitioning ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">
                    {activeTab === 'bestsellery'
                      ? 'Brak danych bestsellerów'
                      : activeTab === 'promocje'
                        ? 'Brak produktów w promocji'
                        : activeTab === 'polecane'
