...
async function updateSessionMetrics(
  ...
  try {
    ...
    const existingMetrics = (await redisCache.get<SessionMetricsState>(
      metricsKey
    )) ?? {
      page_views: 0,
      events_count: 0,
...
// Process real-time metrics
async function processRealTimeMetrics(
  events: AnalyticsEvent[],
  _sessionId: string
) {
  try {
    ...
    const updatedKeys = [
      ...existingKeys.filter(key => key !== realTimeKey),
      realTimeKey,
    ].slice(-200);
...
async function updateGlobalMetrics(metrics: RealTimeMetrics) {
  try {
    ...
    const globalMetrics = (await redisCache.get<GlobalMetricsState>(
      globalKey
    )) ?? {
      total_sessions: 0,
      total_events: 0,
    ...
    Object.entries(metrics.event_types).forEach(([type, count]) => {
      globalMetrics.event_types[type] =
        (globalMetrics.event_types[type] || 0) + count;
    });
...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (type) {
      case 'summary':
        response = await getAnalyticsSummary();
        break;
      case 'realtime':
        response = await getRealTimeMetrics();
        break;
      case 'sessions':
...
async function getSessionData() {
  try {
    // Get active sessions
    const sessionKeys =
      ((await redisCache.get('analytics:session:*')) as string[]) || [];
    const sessions = [];
