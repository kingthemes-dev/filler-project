    ...
    (set, get) => ({
      ...
      addItem: async item => {
        try {
          ...
          // Sync with WooCommerce API via /api/cart-proxy (non-blocking)
          try {
            ...
            if (response.ok) {
              ...
              if (apiResponse.success && apiResponse.data?.cart) {
                // Optionally sync cart data from WooCommerce response
                logger.debug('CartStore: Item synced with WooCommerce', {
                  itemId: item.id,
                });
              }
            } else {
              logger.debug('CartStore: WooCommerce sync failed (non-critical)', {
                status: response.status,
              });
            }
          } catch (error) {
            // Non-blocking: cart works locally even if API fails
            logger.debug('CartStore: WooCommerce sync error (non-critical)', {
      ...
      removeItem: async (itemId, variantId) => {
        try {
          ...
          logger.warn(
            'CartStore: Error removing item, falling back to local removal',
            {
              error: error instanceof Error ? error.message : error,
              itemId,
              variantId,
      ...
      updateQuantity: async (itemId, quantity, variantId) => {
        ...
        try {
          ...
          logger.warn(
            'CartStore: Error updating quantity, falling back to local update',
            {
              error: error instanceof Error ? error.message : error,
              itemId,
              variantId,
